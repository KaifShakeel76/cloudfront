resource "aws_cloudfront_distribution" "cloudfront_distribution" {
  ...
  content {
    ...
    dynamic "member" {
      // Agar 2 ya zyada members nahi hain, toh 2 default members add karenge
      for_each = length(origin_group.value.member) >= 2 ? origin_group.value.member : [
        { origin_id = origin_group.value.id },
        { origin_id = "fallback-origin-id" }
      ]
      content {
  ...
  default_cache_behavior {
    ...
    dynamic "forwarded_values" {
      ...
      content {
        query_string = var.forward_query_string  # Ensure this is a boolean
        headers      = var.forward_header_values   # This can be a list or map based on your setup

        cookies {
          forward = var.forward_cookies  # This should be "none", "all", or "whitelist"
        }
      }
    }
